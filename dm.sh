    ...
    if ! command -v docker >/dev/null 2>&1; then
        ...
        if [ "$PACKAGE_MANAGER" = "yum" ]; then
            if [ -n "$custom_docker_dir" ]; then
                local docker_install_cmd="bash <(curl -sSL https://raw.githubusercontent.com/jaman21/docker-manager/main/di.sh) --close-firewall true --data-root $custom_docker_dir"
            else
                local docker_install_cmd="bash <(curl -sSL https://raw.githubusercontent.com/jaman21/docker-manager/main/di.sh) --close-firewall true"
            fi
            ...
            if [ -n "$custom_docker_dir" ]; then
                local docker_install_cmd="bash <(curl -sSL https://raw.githubusercontent.com/jaman21/docker-manager/main/di.sh) --data-root $custom_docker_dir"
            else
        ...
        *)
            ...
            if [ -n "$DEFAULT_IFACE" ]; then
                ...
                if [ -n "$IP_INFO" ]; then
                    ...
                    echo "ip addr add $IP_ADDR/$NETMASK dev $DEFAULT_IFACE 2>/dev/null || true" >> "$mount_point/opt/env.sh"
                fi

                GATEWAY=$(ip route | grep default | awk '{print $3}' | head -1)
                if [ -n "$GATEWAY" ]; then
                    echo "ip route add default via $GATEWAY 2>/dev/null || true" >> "$mount_point/opt/env.sh"
                fi
            fi

            DNS_SERVERS=$(cat /etc/resolv.conf | grep nameserver | awk '{print $2}' | head -1)
            if [ -n "$DNS_SERVERS" ]; then
                echo "echo 'nameserver $DNS_SERVERS' > /etc/resolv.conf 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            fi

            echo "mkdir -p /dev 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "mkdir -p /dev/pts 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            mount -t proc proc "$mount_point/proc" 2>/dev/null || true
            mount -t sysfs sys "$mount_point/sys" 2>/dev/null || true
            mount -t devtmpfs dev "$mount_point/dev" 2>/dev/null || true
            mount -t devpts pts "$mount_point/dev/pts" 2>/dev/null || true
            mount -t tmpfs shm "$mount_point/dev/shm" 2>/dev/null || true
            mount --bind /tmp "$mount_point/tmp" 2>/dev/null || true

            echo "export LANG=C.UTF-8 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "export LC_ALL=C.UTF-8 2>/dev/null || true" >> "$mount_point/opt/env.sh"

            echo "if command -v apt-get >/dev/null 2>&1; then" >> "$mount_point/opt/env.sh"
            echo "  echo 'APT::Get::AllowUnauthenticated \"true\";' > /etc/apt/apt.conf.d/99allow-unauth 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "  echo 'Acquire::Check-Valid-Until \"false\";' >> /etc/apt/apt.conf.d/99allow-unauth 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "  echo 'APT::Get::AllowDowngrade \"true\";' >> /etc/apt/apt.conf.d/99allow-unauth 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "  apt-get update --allow-unauthenticated --allow-insecure-repositories 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "elif command -v yum >/dev/null 2>&1; then" >> "$mount_point/opt/env.sh"
            echo "  yum update --assumeyes --skip-broken 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "elif command -v dnf >/dev/null 2>&1; then" >> "$mount_point/opt/env.sh"
            echo "  dnf update --assumeyes --skip-broken 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "elif command -v apk >/dev/null 2>&1; then" >> "$mount_point/opt/env.sh"
            echo "  apk update 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "elif command -v pacman >/dev/null 2>&1; then" >> "$mount_point/opt/env.sh"
            echo "  pacman -Sy --noconfirm 2>/dev/null || true" >> "$mount_point/opt/env.sh"
            echo "elif command -v zypper >/dev/null 2>&1; then" >> "$mount_point/opt/env.sh"
            echo "  zypper refresh --non-interactive 2>/dev/null || true" >> "$mount_point/opt/env.sh"
...
install_container() {
    ...
    while [[ -z "$full_image_name" ]]; do
        read -p "enter complete image name (e.g. ubuntu:latest) (q to return): " full_image_name
